...
static const char *LOG_TAG = "XBOXONESParser";
...
uint16_t XBOXONESParser::getButtonPress(ButtonEnum b) {
        ...
        return xboxOneSData.btn.val & (1UL << XBOX_ONE_S_BUTTONS[index]);
}
...
bool XBOXONESParser::getButtonClick(ButtonEnum b) {
        ...
        uint32_t mask = 1UL << XBOX_ONE_S_BUTTONS[index];
...
void XBOXONESParser::Parse(uint8_t len, uint8_t *buf) {
        if (len > 1 && buf)  {
                ...
                if (buf[0] == 0x01) // Check report ID
                        memcpy(&xboxOneSData, buf + 1, min((uint8_t)(len - 1), sizeof(xboxOneSData)));
                        ...
                        ESP_LOGD(LOG_TAG,"XBOXONES Unknown report id: %02X", buf[0]);
                        return;
                }
