...
static const char *LOG_TAG = "PS5Parser";
...
bool PS5Parser::getButtonPress(ButtonEnum b) {
        ...
        if (index <= LEFT) // Dpad
                ...
                return ps5Data.btn.val & (1UL << PS5_BUTTONS[index]);
}

bool PS5Parser::getButtonClick(ButtonEnum b) {
        const int8_t index = getButtonIndexPS5(b); if (index < 0) return 0;
        uint32_t mask = 1UL << PS5_BUTTONS[index];
...
void PS5Parser::Parse(uint8_t len, uint8_t *buf) {
        if (len > 1 && buf)  {
                ...
                if (buf[0] == 0x01) // Check report ID
                        ...
                        if (len < 3) {
                                ESP_LOGD(LOG_TAG,"PS5 Report is too short: %d", len);
                                return;
                        }
                        memcpy(&ps5Data, buf + 2, min((uint8_t)(len - 2),  sizeof(ps5Data)));
                } else {
                        ESP_LOGD(LOG_TAG,"PS5 Unknown report id: %02X", buf[0]);
                        return;
                }
