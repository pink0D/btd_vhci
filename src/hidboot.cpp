...
void MouseReportParser::Parse(bool is_rpt_id __attribute__((unused)), uint8_t len __attribute__((unused)), uint8_t *buf) {
...
void KeyboardReportParser::Parse(bool is_rpt_id __attribute__((unused)), uint8_t len __attribute__((unused)), uint8_t *buf) {
        // On error - return
        if (buf[2] == 1)
                return;
        ...
        for (uint8_t i = 2; i < 8; i++) {
                ...
                if (!down) {
                        //HandleLockingKeys(hid, buf[i]); // usb only, not needed for BT!
                        OnKeyDown(*buf, buf[i]);
                }
                if (!up)
...
uint8_t KeyboardReportParser::OemToAscii(uint8_t mod, uint8_t key) {
        ...
        if (VALUE_WITHIN(key, 0x04, 0x1d)) {
                ...
                return ((shift) ? getSymKeysUp()[key - 0x2d] : getSymKeysLo()[key - 0x2d]);
        else if(VALUE_WITHIN(key, 0x54, 0x58))
                return getPadKeys()[key - 0x54];
        else {
                switch(key) {
